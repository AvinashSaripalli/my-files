...
exports.createTask = (req, res) => {
  const { companyName, name, description, dueDate, createdBy, employeeIds, status, isRecurring } = req.body;
  const insertTaskSql = `
    INSERT INTO tasks (companyName, name, description, dueDate, createdBy, status, isRecurring)
    VALUES (?, ?, ?, ?, ?, ?, ?)
  `;
  db.query(insertTaskSql, [companyName, name, description, dueDate, createdBy, status || 'Pending', isRecurring || false], (err, result) => {
    if (err) {
...
exports.getTasks = (req, res) => {
  ...
  const sql = `
    SELECT 
      t.id, t.name, t.description, t.dueDate, t.createdBy, t.status, t.createdOn, t.isRecurring,
...
exports.updateTask = (req, res) => {
  const { id } = req.params;
  const { name, description, dueDate, createdBy, employeeIds, status, isRecurring } = req.body;
  const updateTaskSql = `
    UPDATE tasks 
    SET name = ?, description = ?, dueDate = ?, createdBy = ?, status = ?, isRecurring = ?
    WHERE id = ?
  `;
  db.query(updateTaskSql, [name, description, dueDate, createdBy, status, isRecurring || false, id], (err, result) => {
    if (err) {
...
exports.deleteTask = (req, res) => {
  const { id } = req.params;
  const checkRecurringSql = `SELECT isRecurring FROM tasks WHERE id = ?`;
  db.query(checkRecurringSql, [id], (err, results) => {
    if (err) {
    ...
    if (results[0].isRecurring) {
      return res.status(400).json({ error: 'Cannot delete recurring task directly. Please update it to non-recurring first.' });
    }
