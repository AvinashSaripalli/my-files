...
//         privacyType,
//         createdBy,
//         createdByName: `${workgroup.creatorFirstName || ''} ${workgroup.creatorLastName || ''}`.trim() || createdBy,
//         employees: new Map(),
//       };
//     }
//     if (employeeId && !acc[id].employees.has(employeeId)) {
//       acc[id].employees.set(employeeId, {
//         employeeId,
//         firstName: workgroup.employeeFirstName,
//         lastName: workgroup.employeeLastName,
//         email: workgroup.email,
//         designation: workgroup.designation,
//         department: workgroup.department,
//         technicalSkills: workgroup.technicalSkills,
  ...
  const groupedWorkgroups = workgroups.reduce((acc, workgroup) => {
    ...
    if (!acc[id]) {
      acc[id] = {
        ...
        createdBy,
        createdByName: `${workgroup.creatorFirstName || ''} ${workgroup.creatorLastName || ''}`.trim() || createdBy,
        creatorPhoto: workgroup.creatorPhoto,
        employees: new Map(),
      };
    }
    if (employeeId && !acc[id].employees.has(employeeId)) {
                ...
                <TableRow key={group.id}>
                  ...
                  <TableCell align="center">
                    <Chip
                      ...
                      avatar={
                        <Avatar
                          src={group.creatorPhoto ? `http://localhost:5000${group.creatorPhoto}` : undefined}
                          alt={group.createdByName}
                        />
                      }
                      label={group.createdByName || group.createdBy}
                      sx={{ minWidth: '120px' }}
                    />
                  </TableCell>
                  <TableCell align="center">
